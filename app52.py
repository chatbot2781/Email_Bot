#Desk intervention -not processing the sent_by_sd_team labels.
import os
import re
import json
import base64
...
# ---------------- SD MANUAL LABEL CONFIG ---------------- #
SD_MANUAL_LABEL_NAME = os.getenv("SD_MANUAL_LABEL_NAME", " ")
sd_manual_label_id = None  # Resolved at runtime
...
# ---------------- ALLOWED SENDER DOMAINS CONFIG ---------------- #
ALLOWED_SENDER_DOMAINS = [
    d.strip().lower()
    for d in (os.getenv("ALLOWED_SENDER_DOMAINS", "gmail.com,yahoo.com,outlook.com")).split(",")
    if d.strip()
]
print("DEBUG ── ALLOWED_SENDER_DOMAINS:", ALLOWED_SENDER_DOMAINS)
...
# ---------------- BOT PROCESSED LABEL CONFIG ---------------- #
BOT_PROCESSED_LABEL_NAME = os.getenv("BOT_PROCESSED_LABEL_NAME", "")
bot_processed_label_id = None  # Resolved at runtime

def get_bot_processed_label_id(service) -> str:
    """
    Get Gmail label ID for 'Processed_by_bot'.
    Creates the label automatically if it doesn't exist.
    Caches after first call.
    """
    global bot_processed_label_id
    if bot_processed_label_id:
        return bot_processed_label_id
    try:
        result = service.users().labels().list(userId="me").execute()
        for label in result.get("labels", []):
            if label.get("name", "").lower() == BOT_PROCESSED_LABEL_NAME.lower():
                bot_processed_label_id = label["id"]
                app.logger.info(f"✓ Found label '{BOT_PROCESSED_LABEL_NAME}' → ID: {bot_processed_label_id}")
                return bot_processed_label_id

        # Label not found — create it automatically
        app.logger.info(f"Label '{BOT_PROCESSED_LABEL_NAME}' not found. Creating it...")
        created = service.users().labels().create(
            userId="me",
            body={
                "name": BOT_PROCESSED_LABEL_NAME,
        ...
        bot_processed_label_id = created["id"]
        app.logger.info(f"✓ Created label '{BOT_PROCESSED_LABEL_NAME}' → ID: {bot_processed_label_id}")
        return bot_processed_label_id

    except Exception as e:
        app.logger.error(f"Failed to get/create bot processed label: {e}")
        return None


def mark_message_processed_by_bot(service, message_id: str):
    """
    Apply 'Processed_by_bot' label to a message.
    Called after bot successfully processes any email.
    """
    label_id = get_bot_processed_label_id(service)
    if not label_id:
        app.logger.warning("⚠ Could not apply 'Processed_by_bot' label — label ID not found")
        return
    try:
        ...
        app.logger.info(f"✓ Applied 'Processed_by_bot' label to message {message_id}")
    except Exception as e:
        app.logger.error(f"Failed to apply bot processed label to {message_id}: {e}")
# ============================================================================
# SECTION 1: ADD TO TOP OF FILE (with other configs, around line 100)
# ============================================================================

# Reminder system configuration
REMINDER_PENDING_FILE = os.getenv("REMINDER_PENDING_FILE", "reminder_pending.pkl")
REMINDER_DELAY_HOURS = 0.05  #
MAX_REMINDERS_PER_TEMPLATE = 1

reminder_pending = {}


# ============================================================================
# SECTION 2: ADD THESE FUNCTIONS (around line 1700, after template functions)
# ============================================================================

def _load_reminder_pending():
    global reminder_pending
    try:
        with open(REMINDER_PENDING_FILE, "rb") as f:
            reminder_pending = pickle.load(f)
            app.logger.info(f"Loaded {len(reminder_pending)} pending reminders")
    except (FileNotFoundError, EOFError):
        reminder_pending = {}
    except Exception as e:
        app.logger.warning(f"Error loading reminders: {e}")
        reminder_pending = {}

def _save_reminder_pending():
    try:
        with open(REMINDER_PENDING_FILE, "wb") as f:
            pickle.dump(reminder_pending, f)
    except Exception as e:
        app.logger.warning(f"Failed to save reminders: {e}")

def mark_incomplete_template_pending(thread_id, sender, subject, missing_fields, extracted_values):
    if not thread_id:
        return
    if thread_id not in reminder_pending:
        reminder_pending[thread_id] = {
            "sender": sender, "subject": subject,
            "missing_fields": missing_fields, "extracted_values": extracted_values,
            "first_request_time": datetime.now(timezone.utc).isoformat(),
            "last_reminder_time": None, "reminder_count": 0,
        }
        _save_reminder_pending()

def clear_reminder_pending(thread_id):
    if thread_id in reminder_pending:
        del reminder_pending[thread_id]
        _save_reminder_pending()

def is_reminder_due(thread_id):
    if thread_id not in reminder_pending:
        return False
    entry = reminder_pending[thread_id]
    if entry["reminder_count"] >= MAX_REMINDERS_PER_TEMPLATE:
        return False
    try:
        first_request = datetime.fromisoformat(entry["first_request_time"])
        hours_elapsed = (datetime.now(timezone.utc) - first_request).total_seconds() / 3600
        if hours_elapsed < REMINDER_DELAY_HOURS:
            return False
        if entry["last_reminder_time"]:
            last = datetime.fromisoformat(entry["last_reminder_time"])
            if (datetime.now(timezone.utc) - last).total_seconds() / 3600 < 24:
                return False
        return True
    except:
        return False

def mark_reminder_sent(thread_id):
    if thread_id in reminder_pending:
        reminder_pending[thread_id]["last_reminder_time"] = datetime.now(timezone.utc).isoformat()
        reminder_pending[thread_id]["reminder_count"] += 1
        _save_reminder_pending()

def get_threads_needing_reminders():
    threads = []
    now = datetime.now(timezone.utc)
    for tid, entry in reminder_pending.items():
        if is_reminder_due(tid):
            try:
                first = datetime.fromisoformat(entry["first_request_time"])
                days = (now - first).total_seconds() / 86400
                threads.append({
                    "thread_id": tid, "sender": entry["sender"], "subject": entry["subject"],
                    "missing_fields": entry["missing_fields"], "extracted_values": entry["extracted_values"],
...
def get_reminder_email_template(missing_fields, extracted_values, days_elapsed):
    FDN = {"user_name":"User Name / ID / Email","contact_number":"Contact Number","location":"Location",
           "issue_start_date":"Issue Start Date","application_name":"Impacted Application Name",
           "workaround":"Any Workaround Available","previous_incident":"Previous Incident Reference",
           "mirror_id":"Mirror ID","issue_description":"Issue Description"}
    REQ = ["user_name","contact_number","location","issue_start_date","application_name","issue_description"]
    
    def fr(fn, ph=""):
        d, v = FDN.get(fn,fn), (extracted_values.get(fn) or "").strip()
        s = '<span style="color:red;">*</span>' if fn in REQ else ''
        if v:
            return f'<tr><td style="background-color:#d4edda;font-weight:bold;width:45%;padding:10px;border:1px solid #c3e6cb;">✅ {d}: {s}</td><td style="background-color:#d4edda;width:55%;padding:10px;border:1px solid #c3e6cb;color:#155724;font-weight:bold;">{v}</td></tr>'
        h = f'<span style="color:#999;font-size:12px;">{ph}</span>' if ph else ''
        return f'<tr><td style="background-color:#ffe6e6;font-weight:bold;width:45%;padding:10px;border:2px solid #ff4444;">🔴 {d}: {s}</td><td style="background-color:#ffe6e6;width:55%;padding:10px;border:2px solid #ff4444;">{h}</td></tr>'
    
    def dr():
        v = (extracted_values.get("issue_description") or "").strip()
        if v:
            return f'<tr><td colspan="2" style="background-color:#d4edda;padding:10px;border:1px solid #c3e6cb;color:#155724;"><strong>✅ Issue Description:</strong><br/>{v.replace(chr(10),"<br/>")}</td></tr>'
        return '<tr><td colspan="2" style="background-color:#ffe6e6;padding:10px;border:2px solid #ff4444;"><strong>🔴 Issue Description: <span style="color:red;">*</span></strong><br/><span style="color:#999;font-size:12px;">[Please describe your issue in detail here]</span></td></tr>'
    
    mc, ml = len(missing_fields), "".join(f"<li><strong>{f}</strong></li>" for f in missing_fields)
    dt = f"{int(days_elapsed)} day{'s' if days_elapsed>=2 else ''}"
    
    return f'''<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="font-family:Arial,sans-serif;color:#333;margin:20px;">
<div style="background-color:#ffe6e6;border:2px solid #ff4444;border-radius:8px;padding:20px;margin-bottom:20px;">
<h3 style="color:#cc0000;margin-top:0;">⏰ Friendly Reminder: Complete Your Support Request</h3>
<p>Hi! We noticed you started a support request <strong>{dt} ago</strong>, but we're still waiting for required information.</p>
<p><strong>To create your incident, we need:</strong></p><ul style="color:#cc0000;margin:5px 0 0 0;">{ml}</ul>
<p style="margin-top:15px;">Please reply with the missing information so we can help you.</p></div>
<table style="margin-bottom:15px;font-size:13px;"><tr>
<td style="background-color:#d4edda;border:1px solid #c3e6cb;padding:6px 14px;color:#155724;border-radius:4px;">✅ You already provided this</td>
<td style="width:20px;"></td>
<td style="background-color:#ffe6e6;border:2px solid #ff4444;padding:6px 14px;color:#cc0000;border-radius:4px;">🔴 Still need this</td>
</tr></table>
<h3 style="color:#2563eb;">📋 Your Information (complete 🔴 fields and reply)</h3>
<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;max-width:800px;margin:10px 0;">
{fr("user_name")}{fr("contact_number")}{fr("location","Terminal / Building / Landside / Airside")}{fr("issue_start_date")}{fr("application_name")}
{fr("workaround")}{fr("previous_incident")}
<tr><td style="background-color:#f0f0f0;font-weight:bold;padding:10px;border:1px solid #ccc;">Error Screenshot:</td>
<td style="background-color:#fff;padding:10px;border:1px solid #ccc;color:#666;font-size:12px;">(Attach if applicable)</td></tr>
{fr("mirror_id")}</table>
<h3 style="color:#2563eb;margin-top:20px;">Issue Description <span style="color:red;">*</span></h3>
<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;max-width:800px;margin:10px 0;">{dr()}</table>
<p style="color:red;font-size:13px;"><strong>* = Required field</strong></p><hr style="border:none;border-top:2px solid #e0e0e0;margin:25px 0;">
<div style="background-color:#f0f8ff;border:1px solid #4a90e2;border-radius:6px;padding:15px;margin:20px 0;">
<strong>💡 How to Complete:</strong><ol style="margin:10px 0;"><li>Reply to this email</li><li>Fill in 🔴 fields</li><li>Send</li><li>We'll create your incident!</li></ol></div>
<p><strong>Need help?</strong><br/>If you no longer need assistance, ignore this email.<br/>If you have questions, reply and let us know.</p>
<p>Thank you,<br/><strong>Support Team</strong></p></body></html>'''

def process_reminders(service):
    threads = get_threads_needing_reminders()
    if not threads:
        return {"reminders_sent": 0, "reminders_failed": 0}
    app.logger.info(f"Processing {len(threads)} reminders")
    sent, failed = 0, 0
    for t in threads:
        try:
            body = get_reminder_email_template(t["missing_fields"], t["extracted_values"], t["days_elapsed"])
            subj = f"Re: {t['subject']} - Reminder: Complete Your Support Request"
            send_email(service, t["sender"], subj, body, is_html=True)
            mark_reminder_sent(t["thread_id"])
            sent += 1
            app.logger.info(f"✓ Reminder sent to {t['sender']}")
        except Exception as e:
            app.logger.error(f"✗ Reminder failed: {e}")
            failed += 1
    return {"reminders_sent": sent, "reminders_failed": failed}

# Load on startup
_load_reminder_pending()
...
def get_sd_label_id(service) -> str:
    """Get Gmail label ID for 'handled_by_sd_team'. Caches after first call."""
    global sd_manual_label_id
    if sd_manual_label_id:
        return sd_manual_label_id
    try:
        result = service.users().labels().list(userId="me").execute()
        for label in result.get("labels", []):
            if label.get("name", "").lower() == SD_MANUAL_LABEL_NAME.lower():
                sd_manual_label_id = label["id"]
                app.logger.info(f"✓ Found label '{SD_MANUAL_LABEL_NAME}' → ID: {sd_manual_label_id}")
                return sd_manual_label_id
        app.logger.warning(f"⚠ Label '{SD_MANUAL_LABEL_NAME}' not found in Gmail")
    except Exception as e:
        app.logger.error(f"Failed to fetch Gmail labels: {e}")
    return None


def is_thread_handled_by_sd(service, thread_id: str, message_label_ids: list) -> bool:
    """
    Returns True if the thread has the 'handled_by_sd_team' label anywhere.
    Checks current message labels first (fast), then full thread (thorough).
    """
    label_id = get_sd_label_id(service)
    if not label_id:
        return False

    # Fast check: label on this specific message
    if label_id in (message_label_ids or []):
        app.logger.info(f"✓ SD label on message — skipping thread {thread_id}")
        return True

    # Thorough check: any message in the thread has the label
    try:
        thread = service.users().threads().get(
            userId="me", id=thread_id, format="minimal"
        ).execute()
        for thread_msg in thread.get("messages", []):
            if label_id in (thread_msg.get("labelIds", []) or []):
                app.logger.info(f"✓ SD label found in thread — skipping thread {thread_id}")
                return True
    except Exception as e:
        app.logger.warning(f"Failed to check thread labels for {thread_id}: {e}")
...
def is_allowed_sender(sender_header: str) -> bool:
    """
    Returns True if sender's domain is in the ALLOWED_SENDER_DOMAINS list.
    If ALLOWED_SENDER_DOMAINS is empty, all domains are allowed.
    """
    if not ALLOWED_SENDER_DOMAINS:
        return True  # No restriction configured — allow all

    domain = _extract_domain(sender_header).lower()
    if not domain:
        app.logger.warning(f"Could not extract domain from sender: {sender_header}")
        return False

    allowed = domain in ALLOWED_SENDER_DOMAINS
    if not allowed:
        app.logger.info(f"⛔ Sender domain '{domain}' not in allowed list — skipping")
    return allowed
...
def is_template_response(email_body: str, subject: str) -> bool:
    ...
    for marker in template_subject_markers:
        if marker in (subject or "").lower():
            app.logger.info(f"✓ Subject contains '{marker}' - IS template response")
            return True
...
def clean_subject_for_short_description(subject: str) -> str:
    ...
    # Remove leading/trailing asterisks from subject parts
    ...
    cleaned = re.sub(r'\s*-?\s*Reminder: Complete Your Support Request\s*', '', cleaned, flags=re.I)
...
def extract_field_value(email_body: str, field_name: str) -> str:
    ...
    for pattern in patterns.get(field_name, []):
        ...
        # Remove leading asterisks and whitespace (from HTML template labels like "* User Name:")
...
def build_enhanced_description_from_fields(extracted_values: dict) -> str:
    ...
    for field_name in field_order:
        ...
        if value:
            # Strip any stray asterisks that may have come from template labels
            value = re.sub(r'^\*+\s*', '', value).strip()
            display_name = FIELD_DISPLAY_NAMES.get(field_name, field_name)
    ...
    while True:
        ...
        for ref in messages:
            try:
                ...
                # ============ STEP 0A: SKIP THREADS HANDLED BY SD TEAM ============
                if is_thread_handled_by_sd(service, thread_id, message_label_ids):
                    app.logger.info(f"⏭ Skipping — 'handled_by_sd_team' label detected on thread {thread_id}")
                    continue  # ← Do nothing else, move to next mail
                ...
                if has_skip_address:
                    ...
                    # Mark as read
                    try:
                        mark_message_processed_by_bot(service, msg["id"])
                        service.users().messages().modify(
                            userId="me",
                            id=msg["id"],
                ...
                template_subject_triggers = [
                    "additional information required",
                    "please complete required fields",
                    "complete required fields",
                ]
                if any(t in (subject or "").lower() for t in template_subject_triggers):
                    app.logger.info("✓✓✓ SUBJECT TRIGGERS TEMPLATE RESPONSE MODE")
                ...
                if is_template_reply:
                    ...
                    if not is_valid:
                        ...
                        try:
                            ...
                            mark_incomplete_template_pending(thread_id, sender, subject, missing_fields, extracted_values)
                    ...
                    try:
                        ...
                        clear_reminder_pending(thread_id)
...
Support Team"""
    ...
    try:
        reminder_stats = process_reminders(service)
        app.logger.info(f"Reminder processing: {reminder_stats}")
    except Exception as e:
        app.logger.error(f"Reminder processing failed: {e}")
        reminder_stats = {"reminders_sent": 0, "reminders_failed": 0}

    return {
         "items": results,
         "blocked_count": blocked_count,
         "besafe_count": besafe_count,
         "reminders_sent": reminder_stats.get("reminders_sent", 0),
         "reminders_failed": reminder_stats.get("reminders_failed", 0)
    }
